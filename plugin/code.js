      ...
      try {
        ...
        if (textNode.fontName !== figma.mixed) {
          try {
            ...
            log(`경고: 폰트 로드 실패 (${JSON.stringify(textNode.fontName)}), 기본 폰트로 계속 진행...`);
          }
        } else {
        ...
        // 텍스트 업데이트
        // nullish 병합 대신, ES5 방식으로 처리
        let newText = patch.newText;
        if (newText === null || newText === undefined) {
          newText = '';
        }
        textNode.characters = String(newText);
...
function log(text, type = 'info') {
  const logType = type === 'info' ? 'log' : `log-${type}`;
  figma.ui.postMessage({
    type: logType,
    text,
  });
